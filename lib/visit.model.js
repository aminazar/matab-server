...
class Visit extends SqlTable {
      ...
      this.sql.visits.getVisit({vid: vid})
        .then(res => {
          if (res.length === 0)
            ...
            response = res[0];
            response = helpers.convertAllGregorianDates(response);
  ...
  startWaiting(displayName, did, pid, data, immediate_visit = false) {
    return new Promise((resolve, reject) => {
      if(!data.page_number || !data.notebook_number) {
        let err = new Error(`You did not specify page number or notebook number`);
        err.status = 400;
        reject(err);
      } else {
              ...
              .then(() => {
                ...
                return this.saveData(diff)
                  .then(newVid => {
                    ...
                    p.columns.forEach(c => diff[c] = currentVisits[0][p]);
  ...
  undoVisit(uid, displayName, isDoctor, vid) {
    return new Promise((resolve, reject) => {
      return this.sql.visits.checkUndoConditions({vid: vid})//select(['vid'], [], ['start_time'])({vid: vid})
        .then(startedVisit => {
          if(startedVisit.length > 1 || (startedVisit[0] && startedVisit[0].document_id)) {
            let err = new Error(`A visit with handwritten comments cannot be undone.`);
            err.status = 400;
            return Promise.reject(err);
          }
          else if (startedVisit[0]) {
            if (startedVisit[0].did !== +uid && isDoctor) {
      ...
      return this.sql.visits.select(['vid'], ['start_time'])({vid: vid})
        .then(queuedVisit => {
          if (queuedVisit[0]) {
            ...
            let err = new ReferenceError(`Visit with vid=${vid} has been started or not found, so it cannot be marked as no-cardio.`);
            err.status = 400;
            return Promise.reject(err);
          }
        })
