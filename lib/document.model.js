...
class Document extends SqlTable {
  ...
  saveHandscript(userName, fileData, wss) {
    return new Promise((resolve, reject) => {
      ...
      this.sql.visits.getVisitForPage({name: userName, paper_id: (pageNumber - 1) + (notebookNumber - 1) * 101})
        ...
        .then(() => {
          wss.connections
            .filter(c=>c.path.split('/')[2]===userName)
            .forEach(c=>c.send(JSON.stringify({text:"Your handwritten comments were added.",msgType:"Comments saved", sd:sd})));
        })
