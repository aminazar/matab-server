     ...
     this.sql.visits.getVisitForPage({name: userName, paper_id: (pageNumber - 1) + (notebookNumber - 1) * 101})
        .then(res => {
          if (res.length !== 1) {
            ...
            let err = new ReferenceError(`The patient record for this document was not found.`);
            err.status = 400;
            reject(err);
          }
          else {
	  ...
	  let dirPath = path.dirname(sd.local_addr);
          if(res.length > 0){
		console.log(pdftkPathCommand + ' "' + res[0].local_addr.replace(/\\/g,"/") + '" background "' + sd.local_addr.replace(/\\/g,"/") + '" output "' + dirPath + '/combined.pdf"'); 
