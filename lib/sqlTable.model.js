...
class SqlTable{
  ...
  select(data) {
    this.construct(data);
    return this.sql[this.tableName].select(data ? Object.keys(data) : [])(data);
  }

  delete(id) {
    this[this.idMember] = id;
    return this.sql[this.tableName].delete(this);
  }
  ...
  importData(data){
    if(!this.columns)
      ...
      for(let key in this.columns.concat(this.idMember))
