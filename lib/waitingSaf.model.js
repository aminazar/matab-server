...
class WaitingSaf extends SqlTable{
  ...
  deleteFromSaf(pid){
    return this.sql.waiting_q.deleteVisitingPatientFromWaiting_q({pid:pid});
  }

  getNewAddedPatientWaiting(pid,did){
    return this.sql.waiting_q.getNewPatientWaitingById({pid:pid,did:did});
  }
  //
  // deleteLastDayWaitings(current_date){
  //   return this.sql.waiting_q.deleteLastDayWaitings({current_date:current_date});
  // }
}
