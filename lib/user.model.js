...
class User extends SqlTable {
    ...
    loginCheck(username = this.username, password = this.password) {
      return new Promise((resolve, reject) => {
        this.load(username, password)
          .then(() => {
            this.checkPassword()
              .then(() => socket.storeNamespace((username === 'admin' ? 'admin' : (this.is_doctor ? 'doctor' : 'user')) + '/' + username))
              .then(resolve)
    ...
    static deserialize(user, done) {
        ...
        userInstance.display_name = user.display_name;
    ...
    static afterLogin(username, is_doctor, display_name, uid) {
      return new Promise((resolve, reject) => {
        socket.storeNamespace((username === 'admin' ? 'admin' : (is_doctor ? 'doctor' : 'user')) + '/' + username)
          .then(res => {
            resolve({
              user: username,
    ...
    select(is_doctor) {
        return new Promise((resolve, reject) => {
            this.sql.users.select()
                .then(res => {
                    resolve(res.filter(r => r.name !== 'admin' && (is_doctor === undefined || r.is_doctor === is_doctor)).map(r => {
                        delete r.secret;
                        return r
                    }));
                })
